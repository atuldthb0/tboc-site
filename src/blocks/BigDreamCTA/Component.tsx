  ...
  return (
    <section className="relative w-full h-screen flex items-center justify-end overflow-hidden pt-12">
      ...
      <div className="absolute left-8 md:left-16 lg:left-24 top-1/2 -translate-y-1/2 z-10 hidden md:flex flex-col gap-12 lg:gap-16 text-white">
        {trustIndicators?.map((indicator: any, index: number) => (
          <div key={index} className="text-left">
            <div className="text-white font-semibold text-3xl lg:text-4xl mb-2">{indicator.value}</div>
            <div className="text-white/60 text-sm lg:text-base">{indicator?.label}</div>
          </div>
        ))}
      </div>
      ...
      <div className="relative z-10 max-w-7xl w-full px-6 md:px-16 lg:px-24">
        ...
        <div
          className="ml-auto w-[70%] sm:w-[65%] md:max-w-[40rem] text-right pr-4 sm:pr-6 md:pr-12 lg:pr-16 
        ...
        >
          ...
          <h1 className="text-white mb-6 md:mb-12">
            ...
            <span className="block text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-bold leading-[1.1] relative">
              ...
              <span
                className="absolute bottom-0 -right-2 sm:-right-4 md:-right-12 lg:-right-12 
                             w-20 sm:w-28 md:w-40 lg:w-48 h-1 bg-red-500"
              ></span>
            </span>
          </h1>
          ...
          {/* Supporting Text */}
          <p
            className="text-white/90 text-sm sm:text-base md:text-xl lg:text-2xl font-light leading-relaxed 
          ...
          <div className="flex justify-end mb-8 md:mb-0">
            <Link href={ctaButton?.url || '#'}>
              <button
                className="group relative px-5 sm:px-8 md:px-12 py-3 sm:py-4 md:py-6 
          ...
          <div className="md:hidden flex justify-end gap-6 text-white mt-8">
            {trustIndicators?.map((indicator: any, index: number) => (
              <div key={index} className="text-center">
                <div className="text-white font-semibold text-xl sm:text-2xl">{indicator.value}</div>
                <div className="text-white/60 text-xs sm:text-sm">{indicator?.label}</div>
              </div>
            ))}
          </div>
        </div>
